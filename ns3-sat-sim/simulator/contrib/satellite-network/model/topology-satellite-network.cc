...
namespace ns3 {
    ...
    TopologySatelliteNetwork::TopologySatelliteNetwork(Ptr<BasicSimulation> basicSimulation, const Ipv4RoutingHelper& ipv4RoutingHelper) {
        ...
        // Initialize the global orbit-aware threat model (if not already done).
        if (g_orbitAwareThreatModel == 0)
        {
            g_orbitAwareThreatModel = CreateObject<OrbitAwareThreatModel> ();
        }

        // Read optional threat_model_filename from the config.
        // If not present, no attacks will be active.
        std::string threatFilename =
                m_basicSimulation->GetConfigParamOrDefault("threat_model_filename", "");

        if (!threatFilename.empty ())
        {
            g_orbitAwareThreatModel->SetConfig (m_basicSimulation->GetRunDir (), threatFilename);
            g_orbitAwareThreatModel->Load ();
        }
